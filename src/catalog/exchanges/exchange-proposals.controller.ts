...
export class ExchangeProposalsController {
  ...
  @Get('me/access-state')
  @UseGuards(JwtAuthGuard)
  getMyProposalAccessState(
    @CurrentCatalogActor() actor: CatalogActor,
    @Query('requestedItemId') requestedItemId?: string
  ) {
    return this.exchangeProposalsService.getMyProposalAccessState(
      actor,
      requestedItemId
    )
  }

  @Post('me/unlocks/outgoing-ad')
  @UseGuards(JwtAuthGuard)
  @SensitiveRateLimit()
  registerOutgoingAdUnlock(@CurrentCatalogActor() actor: CatalogActor) {
    return this.exchangeProposalsService.registerOutgoingAdUnlock(actor)
  }

  @Post('me/unlocks/incoming/requested-item/:itemId')
  @UseGuards(JwtAuthGuard)
  @SensitiveRateLimit()
  unlockNextIncomingProposal(
    @CurrentCatalogActor() actor: CatalogActor,
    @Param('itemId', new ParseUUIDPipe()) itemId: string
  ) {
    return this.exchangeProposalsService.unlockNextIncomingProposal(
      actor,
      itemId
    )
  }
