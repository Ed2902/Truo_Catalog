...
export class ExchangeProposalsService {
  ...
  async createProposal(
    ...
    const outgoingAccessState =
      await this.negotiationPolicyService.assertRequesterCanCreateAnotherOffer(
      actor.userId,
    ...
    const proposal = await this.prismaService.$transaction(async tx => {
      const createdProposal = await tx.exchangeProposal.create({
        data: {
      ...
      if (
        !outgoingAccessState.isPremium &&
        outgoingAccessState.requiresAdUnlock
      ) {
        await this.negotiationPolicyService.consumeOutgoingAdUnlock(
          actor.userId,
          createdProposal.id,
          tx
        )
  ...
  async acceptProposal(actor: CatalogActor, proposalId: string) {
    ...
    if (updatedProposal.match) {
      const chat = await this.chatApiService.ensureChatForMatch({
        matchId: updatedProposal.match.id,
        proposalId: updatedProposal.id,
        requesterUserId: updatedProposal.requesterUserId,
        targetUserId: updatedProposal.targetUserId,
        requestedItemId: updatedProposal.requestedItemId,
        offeredItemId: updatedProposal.offeredItemId,
        requestedItemSnapshot: this.serializeProposalItem(
          updatedProposal.requestedItem
        ) as Record<string, unknown>,
        offeredItemSnapshot: this.serializeProposalItem(
          updatedProposal.offeredItem
        ) as Record<string, unknown>,
      })
      this.onMatchLifecycleEvent('match_created', updatedProposal.match.id)
      await this.notificationsApiService.publish({
        userIds: [
          updatedProposal.requesterUserId,
          updatedProposal.targetUserId,
        ],
        type: 'NEW_MATCH',
        title: 'Tienes un nuevo match',
        body: `Se abrió un match entre "${updatedProposal.requestedItem.title}" y "${updatedProposal.offeredItem.title}".`,
        data: {
          chatId:
            chat &&
            typeof chat === 'object' &&
        ...
        priority: 'HIGH',
        sourceEvent: 'catalog.match_created',
      })
    }
  ...
  async respondToProposal(
    ...
    await this.assertResponseItemReferencesCanBeLinked(
      actor,
      proposal,
      nextItemReferences
    )
  ...
  async listMyProposals(actor: CatalogActor) {
    ...
    const visibleIncomingProposalIds = actor.isPremium
      ? null
      : await this.negotiationPolicyService.listVisibleIncomingProposalIdsForUser(
          actor.userId
        )
    ...
    const visibleProposals = proposals.filter(proposal => {
      ...
      return visibleIncomingProposalIds?.has(proposal.id) ?? false
    })

    return Promise.all(
      visibleProposals.map(proposal => this.serializeProposal(proposal))
    )
  }

  async getMyProposalAccessState(
    actor: CatalogActor,
    requestedItemId?: string
  ) {
    const outgoing =
      await this.negotiationPolicyService.buildOutgoingProposalAccessState(
        actor.userId,
        null,
        actor.isPremium
      )
    ...
    let incoming: {
      ...
      hiddenOffersCount: number
      unlockedOffersCount: number
      freeVisibleLimit: number
      canUnlockNextOfferWithAd: boolean
      isOwner: boolean
    } | null = null

    if (requestedItemId) {
      const item = await this.itemsService.getOwnedItemOrThrow(actor, requestedItemId)
      const incomingState =
        await this.negotiationPolicyService.buildIncomingProposalAccessState(
          item.id,
          actor.userId,
          actor.isPremium
        )
    ...
    return {
      outgoing: {
        totalOffersCreatedCount: outgoing.totalOffersCreatedCount,
        distinctTargetUsersCount: outgoing.distinctTargetUsersCount,
        nextDistinctTargetUsersCount: outgoing.nextDistinctTargetUsersCount,
        freeOfferLimit: 5,
        freeDistinctTargetUsersLimit: 5,
        availableAdUnlocksCount: outgoing.availableAdUnlocksCount,
        nextOfferRequiresAdUnlock: outgoing.requiresAdUnlock,
        isPremium: outgoing.isPremium,
      },
      incoming,
    }
  }

  async registerOutgoingAdUnlock(actor: CatalogActor) {
    if (actor.isPremium) {
      return this.getMyProposalAccessState(actor)
    }

    await this.negotiationPolicyService.registerOutgoingAdUnlock(actor.userId)
    return this.getMyProposalAccessState(actor)
  }

  async unlockNextIncomingProposal(actor: CatalogActor, itemId: string) {
    const item = await this.itemsService.getOwnedItemOrThrow(actor, itemId)

    if (actor.isPremium) {
      return {
        unlockedProposal: null,
        accessState: await this.getMyProposalAccessState(actor, item.id),
      }
    }

    const unlockedProposalId =
      await this.negotiationPolicyService.unlockNextIncomingProposal(
        item.id,
        actor.userId
      )
    const proposal = await this.getProposalOrThrow(unlockedProposalId)

    return {
      unlockedProposal: await this.serializeProposal(proposal),
      accessState: await this.getMyProposalAccessState(actor, item.id),
    }
  }
  ...
  private async assertResponseItemReferencesCanBeLinked(
    actor: CatalogActor,
    proposal: ExchangeProposalWithRelations,
    references: Array<{ itemType: string; itemId: string }>
  ) {
    ...
    for (const reference of references) {
      if (seenItemIds.has(reference.itemId)) {
        throw new ConflictException(
          'This product has already been used in this negotiation'
        )
      }

      seenItemIds.add(reference.itemId)

      if (
        reference.itemId === proposal.requestedItemId ||
        reference.itemId === proposal.offeredItemId
      ) {
        throw new ConflictException(
          'This product is already part of the base negotiation'
        )
      }

      await this.itemsService.getOwnedActiveItemOrThrow(actor, reference.itemId)
    }
  }
