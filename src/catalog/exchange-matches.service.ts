...
export class ExchangeMatchesService {
  ...
  async markMatchCompleted(
    ...
    const result = await this.prismaService.$transaction(async (tx) => {
      ...
      if (activeMatchesToCancel.length > 0) {
        ...
        await tx.exchangeProposal.updateMany({
          where: {
            id: {
              in: activeMatchesToCancel.map((activeMatch) => activeMatch.proposalId),
            },
          },
          data: {
    ...
    await Promise.all(
      affectedItemIds.map((itemId) => this.itemsService.syncNegotiationStatus(itemId)),
    );
  ...
  private assertMatchIsActive(match: Pick<ExchangeMatchWithRelations, 'status'>) {
